...
> - **API-NAMING TRAP:** the same silicon ships under two software ecosystems with *different* BLE API names. Always confirm which one your project uses (§V) before copy-pasting.
...
> 🔑 **İKİ EKOSİSTEM, İKİ İSİMLENDİRME.** Aynı silikon iki SDK ile gelir. Kodun hangisini kullandığını **önce** belirle.

| | Legacy: STSW-BNRGLP-DK | Modern: STM32CubeWB0 |
|---|------------------------|----------------------|
| Header düzeni | aile-başına `Middlewares/ST/External_micro/SimpleBlueNRG-LP_HCI/includes/`: `bluenrg_lp_gap_aci.h`, `..._gatt_aci.h`, `..._hal_aci.h`, `..._l2cap_aci.h`, `bluenrg_lp_aci.h` | **tek autogenerated `ble_api.h`** (+ `ble_stack.h`, `ble_events.h`) |
| İşlem döngüsü | `BTLE_StackTick()` | **`BLE_STACK_Tick()`** / `BLE_STACK_TickNoEvents()` |
| GATT sunucu | `aci_gatt_init`, `aci_gatt_add_serv`, `aci_gatt_update_char_value` | **`aci_gatt_srv_init`, `aci_gatt_srv_add_service`, `aci_gatt_srv_add_char`, `aci_gatt_srv_notify`** |
| GATT istemci | `aci_gatt_disc_*`, `aci_gatt_read_*` | **`aci_gatt_clt_disc_*`, `aci_gatt_clt_read`, `aci_gatt_clt_write`** |
| Stack lib (v3.2a) | `libbluenrg_lp_stack.a` (host+controller) / `libbluenrg_lp_stack_controller_only.a` | `stm32wb0x_ble_stack.a` / `..._controller_only.a` |

Ortak (her iki SDK): `aci_gap_*`, `aci_hal_*`, `aci_l2cap_*`, `hci_*`/`hci_le_*`, `BLE_STACK_Init()`, `tBleStatus`.

### V.1 Stack init (STM32CubeWB0, `ble_stack.h`)

```c
/* ble_stack.h — alanlar SDK sürümüne göre değişir, gh ile doğrula */
BLE_STACK_InitTypeDef p = {
    .BLEStartRamAddress  = (uint8_t*)dyn_alloc_buffer,  /* 32-bit hizalı */
    .TotalBufferSize     = BLE_STACK_TOTAL_BUFFER_SIZE(...),
    .NumAttrRecords      = CFG_BLE_NUM_GATT_ATTRIBUTES,
    .NumOfRadioTasks     = CFG_NUM_RADIO_TASKS,   /* eşzamanlı link sayısı */
    .ATT_MTU             = CFG_BLE_ATT_MTU_MAX,   /* 23..1020 */
    .SleepClockAccuracy  = CFG_BLE_SLEEP_CLOCK_ACCURACY,  /* ppm */
    /* ext-adv / periodic-adv / CTE / ISO / L2CAP CoC alanları... */
};
...
/* Ana döngü */
while (1) {
    BLE_STACK_Tick();                 /* host işleme + tüm callback'ler */
    if (BLE_STACK_SleepCheck() != SLEEPMODE_RUNNING) {
        HAL_PWR_MNGR_Request(...);     /* uyu (§VII) */
    }
}
/* ISR'den Tick ÇAĞIRMA — sadece BLE_STACK_ProcessRequest() güvenli */
```

> `gh search code 'BLE_STACK_InitTypeDef' --owner=STMicroelectronics` →
> `STM32CubeWB0:Middlewares/ST/STM32_BLE/stack/include/ble_stack.h` (alan sırası burada kanonik).

### V.2 Modüler stack yapılandırması

Her özellik bir `*_ENABLED` makrosuyla derlenir (`ble_stack_user_cfg.h` + `config/ble_stack_user_cfg.c`), uygulamadaki `app_conf.h` `CFG_BLE_*` değerlerine bağlanır:
`CONTROLLER_SCAN_ENABLED`, `CONTROLLER_PRIVACY_ENABLED`, `SECURE_CONNECTIONS_ENABLED`, `CONTROLLER_DATA_LENGTH_EXTENSION_ENABLED`, `CONTROLLER_2M_CODED_PHY_ENABLED`, `CONTROLLER_EXT_ADV_SCAN_ENABLED`, `L2CAP_COS_ENABLED`, `CONTROLLER_PERIODIC_ADV_ENABLED`, `CONTROLLER_PERIODIC_ADV_WR_ENABLED` (PAwR), `CONTROLLER_CTE_ENABLED`, `CONTROLLER_POWER_CONTROL_ENABLED`, `CONTROLLER_CIS/BIS_ENABLED`, `CONNECTION_SUBRATING_ENABLED`.
İki link konfigürasyonu iki prebuilt lib'e eşlenir: tam = `stm32wb0x_ble_stack.a`; sadece-controller (`BLESTACK_CONTROLLER_ONLY==1`) = `stm32wb0x_ble_stack_controller_only.a`. **Flash/RAM küçültmenin ana kolu budur** — kullanmadığın profili `0` yap.

### V.3 Event modeli (`ble_events.h`)

Legacy DK'da `void aci_*_event(...)` callback prototipleri vardı. WB0'da stack `BLE_STACK_Event(hci_pckt*, len)` çağırır; üretilen dispatch (`ble_events.c`/`app_ble.c`) ecode'a göre çözüp `*_event_rp0` struct'ına cast eder (ör. `hci_le_connection_complete_event_rp0`, `aci_gap_pairing_complete_event_rp0`).

> Bağlantı/PHY/MTU/throughput state machine'leri ve ACI komut örnekleri için [ref-ble-bluenrg355.md](ref-ble-bluenrg355.md) (legacy ACI isimleriyle) — WB0'da `aci_gatt_*` → `aci_gatt_srv_*/clt_*` dönüşümünü uygula.
...
| Şimdiki zaman (STU) | `TIMER_GetCurrentSysTime()` | `HAL_RADIO_TIMER_GetCurrentSysTime()` |
...
# Yeni srv/clt GATT isimleri (WB0) vs legacy
gh search code 'aci_gatt_srv_add_char'  --owner=STMicroelectronics --json repository,path  # WB0
gh search code 'aci_gatt_update_char_value' --owner=STMicroelectronics --json repository,path  # legacy DK
...
**BLE stack**
- [ ] Doğru ekosistem/API isimlendirmesi kullanılıyor (legacy DK vs WB0 — §V). `BLE_STACK_Tick` mi `BTLE_StackTick` mi?
